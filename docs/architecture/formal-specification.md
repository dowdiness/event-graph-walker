...
> 2. **RLE module not in tree** — Laws and references citing `event-graph-walker/rle/...` (L1, L2, §1 layer) describe a planned standalone RLE module that was **never merged**. Those sections are **aspirational / unverified** until the RLE decision is resolved. See [RLE Design Plan](rle-design.md) and [decisions-needed](../internals/roadmaps/decisions-needed.md).
>
