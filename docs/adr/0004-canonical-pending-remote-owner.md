...
OpLog returns an opaque, single-use `PreparedAdmission` containing pending additions, the complete compatibility-ordered operations, and a planner generation token. Document can inspect a read-only view of those operations for complete preflight, but cannot construct or mutate the prepared admission. On success, Document gives that same prepared admission back to OpLog for commit; OpLog does not reprepare from raw incoming operations. A failed preflight registers none of the incoming operations; it may still issue an explicit rejection transition that removes invalid roots and their transitive dependents from existing pending membership. Commit rejects reuse or a generation mismatch rather than applying a stale prepared admission.
...
rather than a per-operation hash set. `PreparedAdmission` keeps its public
...
Differential tests must cover compatibility order, complete-admission preflight, admission failure, rejection and retry, stale queue entries, and compaction with live pending operations. Release-mode planner benchmarks on wasm-gc and JS must compare the test-only fixed-point oracle and new planner in the same process and scenario. The new planner must be at least 50 times faster for a reverse chain of 10,000 operations and 10 times faster for four reverse chains of 250, while small and in-order workloads show no median regression greater than 20%. The integrated Document reverse-chain workload at 10,000 operations must improve by at least 5 times against a baseline recorded immediately before cutover on the same machine with the same command; no benchmark-only legacy admission adapter is added across the Document/OpLog boundary. Compaction must not restore fixed-point scanning to the hot path.
