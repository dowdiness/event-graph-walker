...
ADR 0006 rejected all public pre-mutation footprint APIs and identified an internal prepared-local-mutation design as the preferred research direction, conditional on nine pre-adoption requirements. Issue #107 implemented a test-only Phase A model (`container/prepared_local_mutation_model_wbtest.mbt`) and a correctness suite (`container/prepared_local_mutation_wbtest.mbt`) covering the four-operation tree/property append on the tree/property slice. The Phase A benchmark report (`docs/benchmarks/2026-08-03-prepared-local-mutation-phase-a-cost.md`) characterizes latency and memory cost.
...
All numbers are from `docs/benchmarks/2026-08-03-prepared-local-mutation-phase-a-cost.md` and the test files cited above.
...
- A separate, narrowly scoped follow-up research issue addressing the architectural consequences above, with its own evidence plan.
