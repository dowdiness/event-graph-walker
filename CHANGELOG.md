...
- Consolidated remote admission's cache detachment, projection publication, and
  recovery in the internal projection workspace. Exact indexes and effects now
  share one candidate state; snapshot fallback retains neither. Public APIs and
  authority commit/recovery behavior are unchanged.
...
- Text synchronization now uses schema 2 for both `SyncMessage` and `Version`.
  ...
  sequence ranges.
- Version encoding and decoding are bounded at 512 KiB, 4,096 frontier entries,
  4,096 agent entries, and 4,096 total ranges. Encoding raises `TextError`
...
- Updated `dowdiness/btree` from 0.1.0 to 0.2.0 and `dowdiness/order-tree`
  from 0.1.1 to 0.1.2.
