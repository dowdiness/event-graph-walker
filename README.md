...
- `children(id)`, `is_alive(id)`, `set_property(id, key, value)`, `get_property(id, key)`, and `properties(id)` inspect or annotate nodes. `properties(id)` returns a deterministic `Array[(String, String)]` snapshot of the current winning key/value pairs in sorted key order, or `[]` for unknown or missing nodes.
...
- `set_property(block, key, value)`, `get_property(block, key)`, and `properties(block)` annotate nodes. `properties(block)` returns a deterministic `Array[(String, String)]` snapshot of the current winning key/value pairs in sorted key order, or `[]` for unknown or missing blocks.
